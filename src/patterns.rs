...
fn built_in_patterns() -> Vec<SecretPattern> {
    let defs = vec![
        ...
        ("aws-secret-key", r##"(?i)(?:aws_secret_access_key|aws_secret_key|secret_access_key)\s*[=:]\s*['"]?[A-Za-z0-9/+=]{40}['"]?"##),
        ...
        ("connection-string", r##"(?i)(?:mysql|postgres(?:ql)?|mongodb(?:\+srv)?|redis|amqp|mssql)://[^\s'"]{10,}"##),
        ...
        ("google-oauth-secret", r##"(?i)client_secret['"]?\s*[=:]\s*['"]?GOCSPX-[A-Za-z0-9\-_]+"##),

        // npm
        ("npm-token", r"npm_[A-Za-z0-9]{36}"),

        // Generic API key assignment
        ("generic-api-key", r##"(?i)(?:api_key|apikey|api_secret|secret_key|access_token)\s*[=:]\s*['"][A-Za-z0-9\-_./+=]{20,}['"]"##),
        ...
        // SendGrid
        ("sendgrid-key", r"SG\.[A-Za-z0-9\-_]{22,}\.[A-Za-z0-9\-_]{22,}"),
    ];
...
mod tests {
    ...
    fn check(pattern_name: &str, positives: &[&str], negatives: &[&str]) {
        ...
        for s in negatives {
            assert!(!pat.regex.is_match(s), "{pattern_name} should NOT match: {s}");
        }
    }
    ...
    fn gitlab_token() {
        check("gitlab-token",
            &["glpat-abcdefghijklmnopqrst", "glpat-ABC_DEF-GHI_123456789012"],
    ...
    fn stripe_key() {
        check("stripe-key",
            &["sk_live_abcdefghijklmnopqrst", "pk_test_1234567890abcdefghij"],
    ...
    fn anthropic_key() {
        check("anthropic-key",
            &["sk-ant-api03-abcdefghijklmnopqrst", "sk-ant-ABCDEFGHIJKLMNOPQRST"],
